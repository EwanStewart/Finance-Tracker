...
def fetch_bank_holidays(
    ...
    holidays = {event["date"] for region in regions for event in data[region]["events"]}
    return holidays
...
def first_working_day_of_month(
    year: int, month: int, holidays: set[str]
) -> datetime.date:
...
def send_email(
    api_key: str, from_addr: str, to_addr: str, today: datetime.date
) -> None:
...
def should_send(today: datetime.date, force: bool) -> bool:
    if force:
        print("Force send is set; bypassing the date check")
        sending = True
    else:
        holidays = fetch_bank_holidays()
        target = first_working_day_of_month(today.year, today.month, holidays)
        sending = today == target
        if not sending:
            print(
                f"today {today.isoformat()} is not the first working day "
                f"{target.isoformat()}; skipping"
            )
    return sending
